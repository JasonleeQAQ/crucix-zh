...
import { CacheManager } from './lib/cache.mjs';
...
const cache = new CacheManager(join(RUNS_DIR, 'cache'));
...
async function runSweepCycle() {
  ...
  try {
    ...
    // Merge translations from previous run
    if (currentData) {
      // News
      const prevNews=new Map();
      (currentData.news||[]).forEach(n=>{if(n._rawTitle)prevNews.set(n._rawTitle,n.title)});
      synthesized.news=(synthesized.news||[]).map(n=>{
        const p=prevNews.get(n._rawTitle||n.title);
        if(p&&p!==(n._rawTitle||n.title))n.title=p;
        return n;
      });
      // Feed
      const prevFeed=new Map();
      (currentData.newsFeed||[]).forEach(f=>{if(f._rawHeadline)prevFeed.set(f._rawHeadline,f.headline)});
      synthesized.newsFeed=(synthesized.newsFeed||[]).map(f=>{
        const p=prevFeed.get(f._rawHeadline||f.headline);
        if(p&&p!==(f._rawHeadline||f.headline))f.headline=p;
        return f;
      });
      // Urgent
      if(synthesized.tg?.urgent&&currentData.tg?.urgent){
        const m=new Map();
        currentData.tg.urgent.forEach(u=>{if(u._rawText)m.set(u._rawText,u.text)});
        synthesized.tg.urgent=synthesized.tg.urgent.map(u=>{
          const p=m.get(u._rawText||u.text);
          if(p&&p!==(u._rawText||u.text))u.text=p;
          return u;
        });
      }
      // TopPosts (cross-match with urgent too)
      if(synthesized.tg?.topPosts){
        const m=new Map();
        if(currentData.tg?.topPosts)currentData.tg.topPosts.forEach(p=>{if(p._rawText)m.set(p._rawText,p.text)});
        const mu=new Map();
        if(currentData.tg?.urgent)currentData.tg.urgent.forEach(u=>{if(u._rawText)mu.set(u._rawText,u.text)});
        synthesized.tg.topPosts=synthesized.tg.topPosts.map(p=>{
          const raw=p._rawText||p.text;if(!raw)return p;
          let prev=m.get(raw);
          if(!prev||prev===raw){for(const[k,v]of mu){if(k&&(raw.startsWith(k.slice(0,30))||k.startsWith(raw.slice(0,30)))){if(v!==k){prev=v;break;}}}}
          if(prev&&prev!==raw)p.text=prev;
          return p;
        });
      }
    }
...
async function start() {
  ...
  server.on('listening', async () => {
    ...
    if (cached) {
      currentData = cached;
      console.log('[Crucix] Loaded from cache — dashboard ready instantly');
      broadcast({ type: 'update', data: currentData });
    } else {
      try {
        ...
        currentData = data;
        cache.save(currentData);
        console.log('[Crucix] Loaded existing data from runs/latest.json');
        broadcast({ type: 'update', data: currentData });
      } catch {
        console.log('[Crucix] No existing data — first sweep required');
      }
    }
    ...
    // Periodic cache save (captures background translation updates)
    setInterval(() => { if (currentData) cache.save(currentData); }, 5 * 60 * 1000);
  });
}
