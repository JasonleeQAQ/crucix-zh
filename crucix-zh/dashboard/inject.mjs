...
import { translateStatic, llmTranslateItems } from '../lib/translate.mjs';
...
export async function synthesize(data) {
  ...
  const tgUrgent = (tgData.urgentPosts || []).filter(p => isEnglish(p.text)).map(p => ({
    channel: p.channel, _rawText: p.text?.substring(0, 200) || "", text: p.text?.substring(0, 200) || "", views: p.views, date: p.date, urgentFlags: p.urgentFlags || []
  }));
  const tgTop = (tgData.topPosts || []).filter(p => isEnglish(p.text)).map(p => ({
    channel: p.channel, _rawText: p.text?.substring(0, 200) || "", text: p.text?.substring(0, 200) || "", views: p.views, date: p.date, urgentFlags: []
  }));
  const who = (data.sources.WHO?.diseaseOutbreakNews || []).slice(0, 10).map(w => ({
    title: translateStatic(w.title?.substring(0, 120) || ''), _rawTitle: w.title?.substring(0, 120) || '', date: w.date, summary: translateStatic(w.summary?.substring(0, 150) || '')
  }));
  ...
  const gdelt = {
    ...
    topTitles: (gdeltData.allArticles || []).slice(0, 5).map(a => translateStatic(a.title?.substring(0, 80) || '')),
  ...
  const markets = {
    indexes: (yfData.indexes || []).map(q => ({
      symbol: q.symbol, name: translateStatic(q.name || q.symbol), price: q.price,
  ...
  // Translate all news titles to Chinese
  ...
  // === LLM Translation: async background ===
  const lp=createLLMProvider(config.llm);
  if(lp?.isConfigured){(async()=>{try{const mb=10;if(news?.length){console.log("[Translate] "+news.length+" news...");await llmTranslateItems(lp,news,"title",mb)}const fd=V2.newsFeed||[];if(fd.length){console.log("[Translate] "+fd.length+" feed...");await llmTranslateItems(lp,fd,"headline",mb)}if(V2.tg?.urgent?.length)await llmTranslateItems(lp,V2.tg.urgent,"text",5);if(V2.tg?.topPosts?.length)await llmTranslateItems(lp,V2.tg.topPosts,"text",5);console.log("[Translate] Complete")}catch(e){console.warn("[Translate] Failed:",e.message)}})()}
...
function buildNewsFeed(rssNews, gdeltData, tgUrgent, tgTop) {
  ...
  for (const n of rssNews) {
    feed.push({
      _rawHeadline: n.title, _rawHeadline: n.title, headline: n.title, source: n.source, type: 'rss',
  ...
  for (const a of (gdeltData.allArticles || []).slice(0, 10)) {
    if (a.title) {
      ...
      feed.push({
        _rawHeadline: a.title.substring(0, 100), headline: a.title.substring(0, 100), source: 'GDELT', type: 'gdelt',
  ...
  for (const p of tgUrgent.slice(0, 10)) {
    ...
    feed.push({
      _rawHeadline: text.substring(0, 100), headline: text.substring(0, 100), source: p.channel?.toUpperCase() || 'TELEGRAM',
...
async function cliInject() {
  ...
  // DISABLED: html = html.replace(/^(let|const) D = .*;\s*$/m, () => 'let D = ' + json + ';');
  // DISABLED writeFileSync(htmlPath, html);
  // DISABLED console.log('Data injected into jarvis.html!');
